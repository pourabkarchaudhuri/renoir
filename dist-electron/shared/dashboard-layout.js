...
import { buildBarChartSvg, buildLineChartSvg, dashboardChartPromptLines, DEFAULT_BAR_SERIES, DEFAULT_LINE_SERIES, inferBarSeries, inferLineSeries, normalizeDashboardCharts, regionExpressesChartData, regionExpressesSecondaryData, svgExpressesChartData, } from './dashboard-charts.js';
...
export const DASHBOARD_REGION_ALIASES = {
 ...
 */
export const DASHBOARD_SCROLL_LOCK_CSS = 'html,body{margin:0!important;padding:0!important;width:100%!important;height:100%!important;' +
...
const PANELS_ROW_WRAP = (inner) => `<div class="panels-row">\n${inner}\n</div>`;
/** Slice a balanced <section> or <div> that carries a canonical data-od-id. */
function sliceOdElement(html, odId) {
    const openRe = new RegExp(`<((?:section)|(?:div))[^>]*data-od-id=["']${odId}["'][^>]*>`, 'i');
...
export function wrapDashboardPanelsRow(html) {
    if (/\bclass=["'][^"']*\bpanels-row\b[^"']*["'][^>]*>[\s\S]*?data-od-id=["']primary-chart["']/i.test(html)) {
        return html;
    }
    const primary = sliceOdElement(html, 'primary-chart');
    const secondary = sliceOdElement(html, 'secondary-panel');
    if (!primary || !secondary || secondary.start < primary.end)
        return html;
    if (!/^\s*$/.test(html.slice(primary.end, secondary.start)))
        return html;
...
export function normalizeDashboardStructure(html) {
    let out = html
        .replace(/<aside\b[^>]*>[\s\S]*?<\/aside>/gi, '')
        .replace(/<nav\b[^>]*\bclass=["'][^"']*\b(?:sidebar|sidenav|rail|nav-rail)\b[^"']*["'][^>]*>[\s\S]*?<\/nav>/gi, '');
    if (/<main[\s>]/i.test(out))
        return out;
    return out.replace(/(<body[^>]*>)([\s\S]*?)(<\/body>)/i, (_match, open, inner, close) => {
        if (/^\s*<main[\s>]/i.test(inner.trim()))
            return `${open}${inner}${close}`;
        return `${open}<main>${inner}</main>${close}`;
    });
}
export function isDashboardArtifact(html) {
    if (new RegExp(`name=["']${DASHBOARD_META}["']`, 'i').test(html))
        return true;
...
function hasRegionId(html, id) {
    if (new RegExp(`data-od-id=["']${id}["']`, 'i').test(html))
        return true;
    const aliases = DASHBOARD_REGION_ALIASES[id] ?? [];
    return aliases.some((alias) => new RegExp(`data-od-id=["']${alias}["']`, 'i').test(html));
}
function hasHeuristicKpis(html) {
    if (hasRegionId(html, 'kpis'))
        return true;
    if (/\bclass=["'][^"']*\b(?:kpis|kpi-grid|metrics-row|stat-grid)\b/i.test(html))
        return true;
    return (html.match(/\bclass=["'][^"']*\bkpi\b/gi) ?? []).length >= 2;
}
function getRegionBlock(html, id) {
    const direct = html.match(new RegExp(`<([a-z]+)[^>]*data-od-id=["']${id}["'][^>]*>[\\s\\S]*?<\\/\\1>`, 'i'))?.[0];
    if (direct)
        return direct;
    for (const alias of DASHBOARD_REGION_ALIASES[id] ?? []) {
        const block = html.match(new RegExp(`<([a-z]+)[^>]*data-od-id=["']${alias}["'][^>]*>[\\s\\S]*?<\\/\\1>`, 'i'))?.[0];
        if (block)
            return block;
    }
    return null;
}
function hasHeuristicPrimaryChart(html) {
    const block = getRegionBlock(html, 'primary-chart');
    if (block)
        return regionExpressesChartData(block);
    const svgs = html.match(/<svg\b[\s\S]*?<\/svg>/gi) ?? [];
    return svgs.some(svgExpressesChartData);
}
function hasHeuristicSecondary(html) {
    const block = getRegionBlock(html, 'secondary-panel');
    if (block)
        return regionExpressesSecondaryData(block);
    if (/\bclass=["'][^"']*\b(?:panel|card)\b[^"']*["'][^>]*>[\s\S]{0,400}<table/i.test(html))
        return true;
    const charts = html.match(/<svg\b/gi) ?? [];
    return charts.length >= 2;
}
export function missingDashboardRegions(html) {
...
export function normalizeDashboardRegionIds(html) {
    let out = html;
    for (const id of REGION_IDS) {
        if (new RegExp(`data-od-id=["']${id}["']`, 'i').test(out))
            continue;
        const aliases = DASHBOARD_REGION_ALIASES[id] ?? [];
...
export function tagHeuristicDashboardRegions(html) {
    ...
    if (!hasRegionId(out, 'kpis')) {
        out = tagOpenTag(out, /<(div|section)([^>]*\bclass=["'][^"']*\b(?:kpis|kpi-grid|metrics-row|stat-grid)\b[^"']*["'][^>]*)>/i, 'kpis');
    }
    if (!hasRegionId(out, 'primary-chart')) {
        let taggedChart = false;
        out = out.replace(/<(div|section)([^>]*\bclass=["'][^"']*(?:chart-panel|chart-area|primary-chart)\b[^"']*["'][^>]*)>/gi, (match, tag, attrs) => {
            if (taggedChart || /data-od-id=/i.test(attrs))
                return match;
            taggedChart = true;
            return `<${tag}${attrs} data-od-id="primary-chart">`;
        });
        if (!hasRegionId(out, 'primary-chart')) {
            out = out.replace(/<(div|section)([^>]*\bclass=["'][^"']*\bchart\b[^"']*["'][^>]*)>/i, (match, tag, attrs) => {
                if (/data-od-id=/i.test(attrs))
                    return match;
                return `<${tag}${attrs} data-od-id="primary-chart">`;
            });
        }
    ...
    if (!hasRegionId(out, 'secondary-panel')) {
        ...
        out = out.replace(/<(div|section)([^>]*\bclass=["'][^"']*\b(?:panel|card)\b[^"']*["'][^>]*)>/gi, (match, tag, attrs, offset) => {
            if (taggedSecondary || /data-od-id=/i.test(attrs))
                return match;
            const after = out.slice(offset, offset + 600);
            if (!/<table/i.test(after) && !/<svg/i.test(after))
                return match;
...
function injectAfterTopbar(html, fragment) {
...
function injectBeforeMainEnd(html, fragment) {
    if (/<\/main>/i.test(html)) {
...
function injectAfterKpis(html, fragment) {
...
export function injectMissingDashboardRegions(html) {
    let out = html;
    if (!hasRegionId(out, 'topbar')) {
        const mainOpen = /<main[^>]*>/i;
 ...
 */
export function ensureDashboardChartData(html) {
    if (!isDashboardArtifact(html) && !hasRegionId(html, 'primary-chart') && !hasRegionId(html, 'kpis')) {
        return html;
    }
    let out = html;
    for (const { id, kind } of [
        { id: 'primary-chart', kind: 'line' },
        { id: 'secondary-panel', kind: 'secondary' },
    ]) {
        ...
        if (!open)
            continue;
        const tag = open[1];
        const attrs = open[2].replace(/data-od-id\s*=\s*["'][^"']+["']/i, `data-od-id="${id}"`);
...
export function ensureDashboardRegions(html) {
...
export function dashboardLayoutPromptLines() {
...
export function injectDashboardShell(html) {
