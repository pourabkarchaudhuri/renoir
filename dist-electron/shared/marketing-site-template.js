...
export function fillMarketingSiteTemplate(template, brief = {}) {
    const name = brief.productName?.trim() || 'Acme';
    let out = template.replace(/Filebase/g, name);
    const tagline = brief.tagline?.trim();
    if (tagline) {
        out = out.replace(/File sync that doesn't eat your bandwidth\./, tagline);
    }
    return out;
}
/** Pull a product name from freeform user text. */
export function inferProductNameFromText(text) {
    const t = text.trim();
    if (!t)
        return 'Acme';
    const beforeDash = t.split(/\s[—–-]\s/)[0]?.trim();
    if (beforeDash && beforeDash.length <= 48)
        return beforeDash;
    const firstLine = t.split(/\n/)[0]?.trim() ?? t;
    return firstLine.slice(0, 48).trim() || 'Acme';
}
export function inferTaglineFromText(text) {
    const t = text.trim();
