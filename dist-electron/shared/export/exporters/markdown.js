...
function yamlFrontmatter(doc) {
    ...
    if (doc.subtitle)
        lines.push(`subtitle: "${doc.subtitle.replace(/"/g, '\\"')}"`);
    if (doc.skillName)
        lines.push(`skill: "${doc.skillName.replace(/"/g, '\\"')}"`);
    if (doc.skillId)
        lines.push(`skillId: "${doc.skillId}"`);
...
async function resolveImageSrc(src, ctx) {
...
async function blockToMd(block, ctx, depth = 0) {
    switch (block.type) {
        case 'heading':
...
export async function documentToMarkdown(doc, ctx) {
    const parts = [yamlFrontmatter(doc), ''];
    if (doc.subtitle)
        parts.push(`*${doc.subtitle}*\n\n`);
    if (doc.description)
        parts.push(`${doc.description}\n\n`);
    if (doc.inputs?.length)
        parts.push(inputsToMd(doc.inputs));
    for (const block of doc.blocks)
        parts.push(await blockToMd(block, ctx));
