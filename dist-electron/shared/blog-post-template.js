...
const DEFAULT_LEDE = 'For two years our Go sync engine was good enough. Then video editors started joining the customer list, and the GC pauses we\'d been politely ignoring turned into bug reports we couldn\'t ignore.';
...
export function fillBlogPostTemplate(template, brief = {}) {
    ...
    if (headline) {
        ...
        out = out.replace(/<title>[^<]*<\/title>/, `<title>${escapeHtml(headline)} — ${escapeHtml(company)}</title>`);
    }
...
export function inferBlogHeadlineFromText(text) {
    const t = text.trim();
    const dashMatch = t.match(/\s[—–-]\s+(.+)/);
    if (dashMatch?.[1])
        return dashMatch[1].trim().slice(0, 120);
    return undefined;
}
export function blogBriefFromText(text) {
    return {
