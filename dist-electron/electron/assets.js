...
export function listProjectAssets(projectId, opts) {
    ...
    const images = listFiles(imgDir, ['.png', '.jpg', '.jpeg', '.webp'])
        .map((p) => {
        const st = statSafe(p);
        return st ? {
    ...
    const videos = listFiles(vidDir, ['.mp4', '.webm', '.mov'])
        .map((p) => {
        const st = statSafe(p);
    ...
    const audio = listFiles(audDir, ['.mp3', '.wav', '.opus', '.ogg', '.m4a'])
        .map((p) => {
        const st = statSafe(p);
        return st ? { url: urlFor(p), savedPath: p, createdAt: st.mtime.toISOString(), sizeBytes: st.size } : null;
    })
