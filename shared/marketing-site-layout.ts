...
export const MARKETING_SCREEN_IDS = ['landing', 'changelog', 'blog'] as const;
export type MarketingScreenId = (typeof MARKETING_SCREEN_IDS)[number];
...
function hasScreenId(html: string, id: string): boolean {
  return new RegExp(`\\bdata-screen-id\\s*=\\s*["']${id}["']`, 'i').test(html);
}

export function parseMarketingScreenIds(html: string): string[] {
  const ids: string[] = [];
...
export function missingMarketingScreens(html: string): MarketingScreenId[] {
...
export function isMarketingSiteArtifact(html: string): boolean {
  if (!html?.trim()) return false;
  if (hasScreenId(html, 'landing')) return true;
...
function wrapLandingScreen(html: string): string {
  if (hasScreenId(html, 'landing')) return html;
...
export function ensureMarketingSiteScreens(html: string): string {
  let out = html?.trim() || '';
  if (!out) return out;
...
export function marketingSitePromptLines(): string[] {
  return [
...
export function lintMarketingSiteFlow(html: string): { rule: string; message: string } | null {
  if (!isMarketingSiteArtifact(html)) return null;
