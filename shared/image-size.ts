...
export const SAFE_IMAGE_SIZES = ['1024x1024'] as const satisfies readonly ImageGenSize[];

export type SafeImageGenSize = (typeof SAFE_IMAGE_SIZES)[number];
 ...
 */
export function parseImageSize(size: string | undefined | null): PixelSize | null {
 ...
 */
export function clampImageSize(size?: string | null): SafeImageGenSize {
  ...
  if (clamped.width <= MAX_IMAGE_DIMENSION && clamped.height <= MAX_IMAGE_DIMENSION) {
    if (clamped.width === 1024 && clamped.height === 1024) return '1024x1024';
  }
  return '1024x1024';
}

/** True when a size string would exceed the per-axis maximum before clamping. */
export function exceedsMaxImageDimension(size?: string | null): boolean {
  const parsed = parseImageSize(size);
