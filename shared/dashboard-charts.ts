...
const DEFAULT_PAD: ChartPadding = { top: 16, right: 16, bottom: 28, left: 44 };
...
function parseChartSeries(raw: string): number[] | null {
  const vals = raw
    .split(',')
    .map((s) => Number(s.trim()))
...
function isValidPolylinePoints(points: string): boolean {
...
function chartSizeFromAttrs(attrs: string, inner: string): { width: number; height: number } {
  ...
  if (w) return { width: Number(w[1]), height: h ? Number(h[1]) : 200 };
  return { width: 600, height: 200 };
}

function chartLabelFromAttrs(attrs: string): string | undefined {
  const m = attrs.match(/\baria-label\s*=\s*["']([^"']*)["']/i);
  return m?.[1];
}

function rectIsValid(tag: string): boolean {
...
export function svgExpressesChartData(svg: string): boolean {
  ...
  if (seriesRaw && parseChartSeries(seriesRaw) && !chartNeedsRebuild(attrs, inner)) return true;
  if (/<polyline\b/i.test(inner)) {
    const poly = inner.match(/<polyline\b[^>]*\bpoints\s*=\s*["']([^"']*)["']/i);
    if (poly && isValidPolylinePoints(poly[1])) return true;
  }
  const rects = [...inner.matchAll(/<rect\b[^>]*>/gi)];
  if (rects.length && rects.every((m) => rectIsValid(m[0]))) return true;
  return false;
}

/** True when a chart region contains at least one SVG with plotted geometry. */
export function regionExpressesChartData(regionHtml: string): boolean {
...
export function regionExpressesSecondaryData(regionHtml: string): boolean {
  if (/<table\b/i.test(regionHtml) && (regionHtml.match(/<tr\b/gi)?.length ?? 0) >= 2) return true;
  return regionExpressesChartData(regionHtml);
}

function parseMetricNumber(text: string): number | null {
  const raw = text.trim();
  if (!raw || !/\d/.test(raw)) return null;
  let mult = 1;
  if (/[Kk]\b/.test(raw) || /\d\s*K\b/i.test(raw)) mult = 1_000;
  if (/[Mm]\b/.test(raw) || /\d\s*M\b/i.test(raw)) mult = 1_000_000;
  const cleaned = raw.replace(/[^0-9.\-]/g, '');
  const n = Number(cleaned);
  return Number.isFinite(n) ? n * mult : null;
}

function extractKpiNumbers(html: string): number[] {
...
function trendSeries(base: number, end: number, steps = 12): number[] {
...
export function inferLineSeries(html: string): number[] {
  const kpis = extractKpiNumbers(html);
  if (kpis.length >= 6) return kpis.slice(0, 12);
  if (kpis.length >= 2) return trendSeries(kpis[0], kpis[kpis.length - 1]);
  if (kpis.length === 1) return trendSeries(kpis[0] * 0.88, kpis[0]);
  return DEFAULT_LINE_SERIES;
}

/** Derive a bar-chart series from KPI numbers when the secondary panel has no data. */
export function inferBarSeries(html: string): number[] {
  const kpis = extractKpiNumbers(html);
  if (kpis.length >= 4) return kpis.slice(0, 7);
  if (kpis.length >= 2) return kpis;
  return DEFAULT_BAR_SERIES;
}

function chartNeedsRebuild(attrs: string, inner: string): boolean {
  const seriesRaw = attrs.match(/data-od-series\s*=\s*["']([^"']+)["']/i)?.[1];
  if (!seriesRaw || !parseChartSeries(seriesRaw)) return false;
...
function plotArea(opts: ChartPlotOptions) {
...
function seriesRange(values: number[]): { min: number; max: number } {
...
export function lineChartPoints(values: number[], opts: ChartPlotOptions = {}): string {
...
export function buildLineChartSvg(values: number[], opts: ChartPlotOptions = {}): string {
...
export function buildBarChartSvg(values: number[], opts: ChartPlotOptions = {}): string {
...
export interface DashboardChartFinding {
  level: 'warn' | 'info';
  rule: string;
  message: string;
}
...
export function lintDashboardCharts(html: string): DashboardChartFinding[] {
  const findings: DashboardChartFinding[] = [];
  if (!isChartLintTarget(html)) return findings;

  for (const region of ['primary-chart', 'secondary-panel'] as const) {
...
function isChartLintTarget(html: string): boolean {
...
function extractRegionHtml(html: string, id: string): string | null {
  const re = new RegExp(
    `<([a-z]+)[^>]*data-od-id=["']${id}["'][^>]*>[\\s\\S]*?<\\/\\1>`,
    'i',
  );
...
export function normalizeDashboardCharts(html: string): string {
  if (!isChartLintTarget(html)) return html;
  return html.replace(/<svg\b([^>]*)>([\s\S]*?)<\/svg>/gi, (full, attrs, inner) => {
    ...
    if (isLine && /preserveAspectRatio\s*=\s*["']none["']/i.test(nextAttrs)) {
      nextAttrs = nextAttrs.replace(
        /preserveAspectRatio\s*=\s*["']none["']/i,
...
export function dashboardChartPromptLines(): string[] {
  return [
