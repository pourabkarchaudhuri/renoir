...
export type BlogPostRegion = (typeof BLOG_POST_REGIONS)[number];

function hasOdId(html: string, id: string): boolean {
  return new RegExp(`\\bdata-od-id\\s*=\\s*["']${id}["']`, 'i').test(html);
}

export function parseBlogPostRegionIds(html: string): string[] {
  const ids: string[] = [];
...
export function missingBlogPostRegions(html: string): BlogPostRegion[] {
  return BLOG_POST_REGIONS.filter((id) => !hasOdId(html, id));
}

/** Heuristic: artifact targets the blog-post single-screen contract. */
export function isBlogPostArtifact(html: string): boolean {
  if (!html?.trim()) return false;
  if (hasOdId(html, 'article-header') && hasOdId(html, 'article-body')) return true;
  return false;
}

/** Prompt block for LLM system prompt when the blog-post skill is active. */
export function blogPostPromptLines(): string[] {
  return [
...
export function lintBlogPostRegions(html: string): { rule: string; message: string } | null {
  if (!isBlogPostArtifact(html)) return null;
