...
function inlineToHtml(inline: ExportInline): string {
  ...
  if (inline.code) t = `<code>${t}</code>`;
  if (inline.bold) t = `<strong>${t}</strong>`;
  if (inline.italic) t = `<em>${t}</em>`;
  return t;
}
...
function inputsToHtml(inputs: ExportInput[]): string {
  if (!inputs.length) return '';
  const rows = inputs.map(
    (i) => `<tr><th>${escHtml(i.label)}</th><td>${escHtml(i.value).replace(/\n/g, '<br>')}</td></tr>`,
  );
...
function blockToHtml(block: ExportBlock): string {
  switch (block.type) {
    ...
    case 'checklist':
      return `<ul class="checklist">${block.items.map(
        (item) => `<li><input type="checkbox" disabled ${item.checked ? 'checked' : ''}> ${escHtml(item.text)}</li>`,
      ).join('')}</ul>`;
    case 'table': {
      ...
      const body = block.rows.map(
        (row) => `<tr>${row.map((c) => `<td>${escHtml(c)}</td>`).join('')}</tr>`,
      ).join('');
...
export function documentToPrintHtml(doc: ExportDocument, theme: ExportTheme = DEFAULT_EXPORT_THEME): string {
  const metaParts: string[] = [];
  if (doc.skillName) metaParts.push(`Skill: ${escHtml(doc.skillName)}`);
...
export const printHtmlExporter: DocumentExporter = {
  format: 'pdf' as const,
...
export function exportDocumentToPrintHtml(doc: ExportDocument, theme?: ExportTheme): string {
