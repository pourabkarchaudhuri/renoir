...
export function injectMermaidScript(html: string): string {
  if (html.includes('mermaid.min.js') || html.includes('mermaid.esm')) return html;
  if (!hasMermaidContent(html)) return html;
  if (/<\/head>/i.test(html)) return html.replace(/<\/head>/i, `${MERMAID_INIT_SCRIPT}\n</head>`);
  if (/<\/body>/i.test(html)) return html.replace(/<\/body>/i, `${MERMAID_INIT_SCRIPT}\n</body>`);
  return html + '\n' + MERMAID_INIT_SCRIPT;
}

/** Wrap artifact HTML with a nav bridge script; idempotent if already wrapped. */
export function wrapWithBridge(html: string, navBridge: string): string {
  if (
    html.includes('id="__renoir_mode_style"')
  ...
  if (/<\/body>/i.test(result)) return result.replace(/<\/body>/i, `${navBridge}</body>`);
  if (/<\/html>/i.test(result)) return result.replace(/<\/html>/i, `${navBridge}</html>`);
  return result + navBridge;
}

/** Bridge-wrapped HTML for offscreen deck export capture. */
export function wrapForExportCapture(html: string): string {
