...
function kmeans(pixels: RGB[], k: number, maxIter = 12): RGB[] {
  if (pixels.length === 0) return [];
  // Seed via spaced sampling
  const seeds: RGB[] = [];
  const step = Math.max(1, Math.floor(pixels.length / k));
  for (let i = 0; i < k; i++) seeds.push({ ...pixels[(i * step) % pixels.length] });
  ...
  for (let iter = 0; iter < maxIter; iter++) {
    ...
    centroids = centroids.map((c, j) => {
      ...
      if (Math.abs(next.r - c.r) + Math.abs(next.g - c.g) + Math.abs(next.b - c.b) > 2) moved++;
      return next;
    });
    if (moved === 0) break;
  }
...
export interface ExtractReq {
  ...
  imageMime?: string;
  k?: number;              // palette size
}

export function extractPalette(req: ExtractReq): ExtractedPalette {
  try {
    ...
    if (pixels.length === 0) return { ok: false, error: 'no opaque pixels' };
    void w; void h;
