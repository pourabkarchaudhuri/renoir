...
export function azureConfig() {
  ...
  return {
    ...
    imageApiKey: process.env.AZURE_IMAGE_API_KEY || process.env.AZURE_FOUNDRY_API_KEY || '',
...
/** Text / LLM fallback via Azure Foundry. */
export function azureConfigured(): boolean {
  const c = azureConfig();
  return Boolean(c.endpoint && c.apiKey);
}

/** Image generation — may use AZURE_IMAGE_ENDPOINT + AZURE_IMAGE_API_KEY. */
export function azureImageConfigured(): boolean {
  const c = azureConfig();
