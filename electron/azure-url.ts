...
export interface AzureRoutes {
  /** Base used for project-rooted v1 endpoints (text-side ops). */
  v1ProjectBase?: string;
  /** Base used for resource-rooted v1 endpoints (images, etc). */
  v1ResourceBase?: string;
  /** Resource root used for legacy `<root>/openai/deployments/...` */
  resourceRoot?: string;
  /** Whether the endpoint was a Foundry project URL */
...
export function parseAzureEndpoint(raw: string): AzureRoutes {
  const trimmed = (raw || '').trim().replace(/\/+$/, '');
  if (!trimmed) return { isProjectScoped: false };

  let url: URL;
  try { url = new URL(trimmed); }
  catch { return { isProjectScoped: false }; }
...
interface DeriveOpts {
  ...
  apiVersion: string;
  deployment: string;
  /** Operation in v1 path syntax, e.g. `images/generations` */
  op: string;
}
 ...
 */
export function deriveAzureUrl(opts: DeriveOpts): string | null {
