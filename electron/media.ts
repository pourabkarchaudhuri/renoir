...
interface AudioReq {
  prompt: string;        // for music or jingle deployments
  text?: string;         // for TTS deployments
  voice?: string;
  format?: 'mp3' | 'wav' | 'opus';
  projectId?: string;
}
...
interface MediaResult {
  ok: boolean;
  files?: { savedPath: string; mime: string; bytes: number }[];
  error?: string;
}

function dirFor(kind: 'audio' | 'video', projectId?: string): string {
  if (projectId) {
...
export async function generateAudio(req: AudioReq): Promise<MediaResult> {
  ...
  if (!deployment) return { ok: false, error: 'AZURE_AUDIO_DEPLOYMENT not set' };
  ...
  if (!url) return { ok: false, error: 'Could not derive audio URL' };
...
export async function generateVideo(req: VideoReq): Promise<MediaResult> {
  if (!azureConfigured()) return { ok: false, error: 'AZURE_NOT_CONFIGURED' };
  const cfg = azureConfig();
  const deployment = process.env.AZURE_VIDEO_DEPLOYMENT || '';
  if (!deployment) return { ok: false, error: 'AZURE_VIDEO_DEPLOYMENT not set in .env' };
  ...
  if (!url) return { ok: false, error: 'Could not derive video URL' };
  ...
  if (ct.includes('application/json')) {
    ...
    if (!dataUrl) return { ok: false, error: 'video response had no url' };
