...
export function ensurePreviewWindow(html: string): { ok: boolean } {
...
<body>
  ...
  detached.webContents.once('did-finish-load', () => {
    if (!detached) return;
    void detached.webContents.executeJavaScript(
      `window.__renoir_preview_html = ${JSON.stringify(html)};`
    );
  });
...
export function pushPreviewHtml(html: string): void {
