...
import type { ExportContext } from '../../shared/export/exporter.js';
import type { CaptureMode, ExportDocument } from '../../shared/export/types.js';
import { inlineResolvableImagesInHtml } from '../../shared/export/inline-export-images.js';
import {
  wrapForExportCapture,
  EXPORT_CAPTURE_FREEZE_SCRIPT,
  EXPORT_SLIDE_FREEZE_FN,
} from '../../shared/preview-nav-bridge.js';
...
async function prepareCaptureHtml(doc: ExportDocument, ctx: ExportContext): Promise<string> {
  const raw = doc.captureHtml ?? doc.previewHtml ?? '';
  if (!raw.trim()) throw new Error('No HTML available for capture.');
...
async function querySlideCount(win: BrowserWindow): Promise<number> {
...
async function waitForSlidePaint(win: BrowserWindow): Promise<void> {
...
function createCaptureWindow(width: number, height: number): BrowserWindow {
...
async function freezeForCapture(win: BrowserWindow, mode: CaptureMode): Promise<void> {
...
async function captureRegionPng(
  ...
  try {
    ...
    try {
      ...
      try { wc.disableDeviceEmulation(); } catch { /* swallow */ }
    }
  } finally {
    if (attached) {
      try { dbg.detach(); } catch { /* swallow */ }
    }
  }
}

/** Capture each slide/screen in present mode as a PNG buffer. */
export async function capturePresentSlides(opts: SlideCaptureOptions): Promise<Buffer[]> {
  ...
  try {
    ...
    await win.webContents.executeJavaScript(
      `window.postMessage({ type: 'renoir:set-mode', mode: 'present' }, '*');`,
    );
    ...
    for (let i = 0; i < total; i += 1) {
      await win.webContents.executeJavaScript(
        `window.postMessage({ type: 'renoir:nav', idx: ${i} }, '*');`,
      );
...
export async function captureArtifactPngs(
  doc: ExportDocument,
  ctx: ExportContext,
): Promise<Buffer[]> {
...
export async function captureDeckSlidePngs(
  doc: ExportDocument,
  ctx: ExportContext,
): Promise<Buffer[]> {
...
export function pngDimensions(buf: Buffer): { width: number; height: number } {
  if (buf.length < 24) return { width: DEFAULT_W, height: DEFAULT_H };
