...
import type { ExportDocument, ExportFormat } from '../../shared/export/types.js';
...
function pageSizeMicrons(widthPx: number, heightPx: number): { width: number; height: number } {
  return {
...
function buildRasterPdfHtml(pngs: Buffer[]): string {
...
function rasterPdfPageSize(pngs: Buffer[]): { width: number; height: number } {
  if (pngs.length === 1) {
...
function ensureExporters(): void {
  if (exportersReady) return;
  ...
  registerExporter({
    ...
    async export(doc, ctx) {
      const pngs = await captureArtifactPngs(doc, ctx);
      if (!pngs.length) throw new Error('No image captured.');
...
export async function exportDocumentToFile(req: ExportDocumentRequest): Promise<ExportDocumentResult> {
  ensureExporters();
