...
import type { ExportDocument, ExportBlock, ExportInline } from '../../shared/export/types.js';
import type { DocumentExporter, ExportContext } from '../../shared/export/exporter.js';
...
function inlineRuns(inlines: ExportInline[], theme = DEFAULT_EXPORT_THEME): (TextRun | ExternalHyperlink)[] {
...
async function blockToDocx(block: ExportBlock, ctx: ExportContext): Promise<(Paragraph | Table)[]> {
...
export async function documentToDocxBuffer(doc: ExportDocument, ctx: ExportContext): Promise<Uint8Array> {
  const theme = ctx.theme;
  ...
  if (doc.skillName) children.push(metaParagraph('Skill', doc.skillName));
