 ...
 */
function adaptDiskPrimer(raw: string): string {
  let primer = raw;
...
function registerIpc(): void {
  ...
  ipcMain.handle('renoir:skills:primer', (_e, id: string) => {
    ...
    if (builtIn) return builtIn.primer;
    const custom = customCatalog.listSkills().find((s) => s.id === id);
    if (custom) return custom.primer;
    ...
    if (disk) return adaptDiskPrimer(disk.primer);
    return null;
  });
  ...
  ipcMain.handle('renoir:custom:deleteSkill', (_e, id: string) => { customCatalog.deleteSkill(id); return { ok: true }; });
  ...
  ipcMain.handle('renoir:custom:deleteSystem', (_e, id: string) => { customCatalog.deleteSystem(id); return { ok: true }; });
  ...
  ipcMain.handle('renoir:custom:deleteDirection', (_e, id: string) => { customCatalog.deleteDirection(id); return { ok: true }; });
  ...
  ipcMain.handle('renoir:theme:set', (_e, theme: 'dark' | 'light') => {
    ...
    for (const w of BrowserWindow.getAllWindows()) {
      try { w.setTitleBarOverlay(overlay); } catch { /* swallow */ }
      try { w.setBackgroundColor(theme === 'light' ? '#f5f6f8' : '#0c0e14'); } catch { /* swallow */ }
    }
    return { ok: true };
  });
  ...
  ipcMain.handle('renoir:agents:cancel', (_e, id: string) => cancelAgent(id));
  ...
  ipcMain.handle('renoir:image:generateBatch', async (_e, req: { items?: any[]; projectId?: string; concurrency?: number }) => {
    ...
    for (const item of req.items) {
      if (!item.prompt || typeof item.prompt !== 'string' || item.prompt.trim() === '') {
        return { ok: false, results: req.items.map((i: any) => ({ id: i.id || '', ok: false, error: 'Each item must have a non-empty prompt' })) };
      }
    }
  ...
  ipcMain.handle('renoir:assets:list', (_e, req: { projectId: string }) =>
    listProjectAssets(req.projectId));
  ...
  ipcMain.handle('renoir:preview:open',  (_e, html: string) => ensurePreviewWindow(html));
  ipcMain.handle('renoir:preview:push',  (_e, html: string) => { pushPreviewHtml(html); return { ok: true }; });
  ipcMain.handle('renoir:preview:isOpen', () => isPreviewOpen());

  // Remix from gallery
  ipcMain.handle('renoir:projects:remix', (_e, req: { skillFromId?: string; systemFromId?: string; artifactFromId?: string; name?: string }) => {
  ...
  // Project rename
  ipcMain.handle('renoir:projects:rename', (_e, req: { id: string; name: string }) => {
  ...
  ipcMain.handle('renoir:projects:addVersion', (_e, req: { id: string; html: string; source?: 'assistant' | 'fork' | 'restore'; note?: string }) => {
    ...
    if (last && last.html === req.html) return { ok: true, project: rec, deduped: true };
    rec.versions.push({
  ...
  ipcMain.handle('renoir:projects:restoreVersion', (_e, req: { id: string; versionId: string }) => {
    const rec = store.getProject(req.id);
    if (!rec || !rec.versions) return { ok: false, error: 'no versions' };
  ...
  ipcMain.handle('renoir:projects:fork', (_e, req: { fromId: string; nameSuffix?: string }) => {
    ...
    const rec: ProjectRecord = {
      ...
      visualDirectionId: (src as any).visualDirectionId,
      conversation: [
        {
          role: 'system',
  ...
  ipcMain.handle('renoir:brand:extract', (_e, text: string) => extractBrandSpec(text || ''));

  // Templates
  ipcMain.handle('renoir:templates:list',   () => listTemplates());
  ipcMain.handle('renoir:templates:save',   (_e, rec) => saveTemplate(rec));
  ipcMain.handle('renoir:templates:delete', (_e, id: string) => { deleteTemplate(id); return { ok: true }; });

  // Projects
  ipcMain.handle('renoir:projects:list',   () => store.listProjects());
  ipcMain.handle('renoir:projects:read',   (_e, id: string) => store.getProject(id) ?? null);
  ipcMain.handle('renoir:projects:create', (_e, meta: Partial<ProjectRecord>) => {
    const now = new Date().toISOString();
    const rec: ProjectRecord = {
  ...
  ipcMain.handle('renoir:projects:save', (_e, rec: ProjectRecord) => {
    rec.updatedAt = new Date().toISOString();
    store.upsertProject(rec);
    return { ok: true };
  });
  ipcMain.handle('renoir:projects:delete', (_e, id: string) => {
    store.deleteProject(id);
    return { ok: true };
  });
  ipcMain.handle('renoir:projects:export', (_e, id: string) => exportProject(id));
