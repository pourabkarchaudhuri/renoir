...
export interface AgentRecord {
  ...
  resolvedPath?: string;
  blurb: string;
  /** stdin mode: how user content reaches the binary */
  invoke: 'stdin' | 'arg' | 'flag';
  /** Extra flags appended before the prompt or to coax non-interactive output */
  args?: string[];
}
...
export function detectAgents(force = false): AgentRecord[] {
  if (cache && !force) return cache;
  cache = KNOWN.map((rec) => {
    const found = whichSync(rec.bin);
    if (found) return { ...rec, available: true, resolvedPath: found };
...
const live = new Map<string, ReturnType<typeof spawn>>();
...
export function invokeAgent(req: InvokeReq): { ok: boolean; error?: string } {
  ...
  const childEnv: Record<string, string | undefined> = { ...process.env, NO_COLOR: '1', TERM: 'dumb' };
  ...
  if (rec.invoke === 'stdin') {
    try { child.stdin.write(req.prompt + '\n'); } catch { /* swallow */ }
...
export function cancelAgent(conversationId: string): boolean {
