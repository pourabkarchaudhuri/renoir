...
export function lintArtifact(html: string): LintReport {
  ...
  for (const rule of RULES) {
    try { const f = rule(html); if (f) findings.push(enrichFinding(f)); }
    catch { /* swallow rule error */ }
  }
  if (isDashboardArtifact(html)) {
    for (const f of lintDashboardCharts(html)) findings.push(enrichFinding(f));
  }
